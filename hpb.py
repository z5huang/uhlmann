...
BX = 0.5
class HPB:
    def __init__(self, nx=50, ny=50, m=M, bx=BX):
    ...
    def hk(self,kx,ky):
        ...
        res = np.sin(kx) * gg1 + np.sin(ky) * gg2 + mk * gg0 + self.bx * ggb
        return res
    ...
    def export_uhlmann(self,mu,t,prepend_rho=True, rot_ky = 0, normalize = False, fn='uhlmann'):
        ...
        with open(fn+'.par', 'w') as par:
            ...
            prepend_rho = %d
            normalize = %d
            """%(self.m,self.bx,self.nx,self.ny,mu,t,rot_ky/np.pi,prepend_rho, normalize)
            print(txt,file=par)

        labels = 'nx kx Tr.amp Tr.phase ' + ' '.join(['amp_%d phase_%d'%(i+1,i+1) for i in range(self.dim)])
        ...
        with open(fn+'.dat', 'w') as dat:
            ...
            for x in np.arange(self.nx):
                ...
                pp = np.angle(uu)
                tr = np.sum(uu)
                print('%d\t%g\t%g\t%g\t%s'%(x,kx,abs(tr),np.angle(tr),'\t'.join([ '%g\t%g'%(a,p) for a,p in zip(aa,pp) ])), file=dat)
        None
    def export_uhlmann_kx(self,mu,kx = None, tlist=np.arange(0.02,0.52,0.02),prepend_rho=True, normalize=True, fn='uhlmann_kc'):
        ...
        with open(fn+'.par', 'w') as par:
            ...
            prepend_rho = %d
            normalize = %d
            """%(kx/np.pi, self.m,self.bx,self.nx,self.ny,mu,prepend_rho, normalize)
            print(txt,file=par)

        labels = 'ny kappa_y T tr.abs tr.angle rho_kappa.tr ' + '\t'.join([ '%d.abs %d.arg'%(i+1,i+1) for i in range(dim) ])
        ...
        with open(fn+'.dat', 'w') as dat:
            ...
            for y in np.arange(self.ny):
                ...
                for t in tlist:
                    ...
                    pp = np.angle(uu)
                    tr = np.sum(uu)
                    print('%d\t%g\t%g\t%g\t%g\t%g\t'%(y,kappa_y,t,abs(tr),np.angle(tr),rho_tr) +
                          '\t'.join([ '%g\t%g'%(aaa,ppp) for aaa,ppp in zip(aa,pp) ]), file=dat)
                dat.write('\n')
        None
